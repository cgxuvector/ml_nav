...
def parse_input():
    ...
    parser.add_argument('--max_frame', type=int, default=1000, help='Maximal number of frames.')
...
def run_agent(max_frame, win_width, win_height, frame_fps, level_name):
    ...
    myEnv = gym.make('DeepmindLabGetTopDownView-v0', width=win_width, height=win_height, observations=["RGBD_INTERLEAVED"])
    # myEnv = gym.make('DeepmindLabLoadRandomMaze-v0', width=win_width, height=win_height, \
    ...
    for ep in tqdm(range(episode_num), desc='Episode loop'):
        ...
        # art0 = arr1[0].imshow(init_obs["RGB.LOOK_LEFT"])
        # arr1[1].set_title("Front view")
        # art1 = arr1[1].imshow(init_obs["RGB.LOOK_FORWARD"])
        # arr1[2].set_title("Right view")
        # art2 = arr1[2].imshow(init_obs["RGB.LOOK_RIGHT"])
        # arr1[3].set_title("Back view")
        # art3 = arr1[3].imshow(init_obs["RGB.LOOK_BACK"])
        ...
        for t in tqdm(range(max_frame), desc="Step loop"):
            ...
            current_obs = current_obs['RGBD_INTERLEAVED']
            ...
            # art0.set_data(current_obs["RGB.LOOK_LEFT"])
            # art1.set_data(current_obs["RGB.LOOK_FORWARD"])
            # art2.set_data(current_obs["RGB.LOOK_RIGHT"])
            # art3.set_data(current_obs["RGB.LOOK_BACK"])
            fig.canvas.draw()
            plt.pause(0.0001)
            if reward == 10:
                break
