...
def image_generation(dataLoader):
    ...
    cvae = DCNets.CVAE(64)
    cvae.load_state_dict(torch.load("/mnt/sda/dataset/ml_nav/model/cvae_c34_h64_b1_L1.pt"))
    cvae.eval()
...
if __name__ == '__main__':
    ...
    # # load the trained model
    # cvae = DCNets.CVAE(64)
    # cvae.load_state_dict(torch.load("/mnt/sda/dataset/ml_nav/model/" + "cvae_" + input_args.model_save_name + ".pt"))
    # cvae.eval()
    #
    # tmp_ori = transformed_dataset.orientation_angle
    # orientations = [torch.from_numpy(tmp_ori[key]).float() for key in tmp_ori.keys()]
    #
    # save_path = "/mnt/sda/dataset/ml_nav/cvae_reconstruction/" + input_args.model_save_name + "_ep50/"
    # for idx, batch in enumerate(dataLoader_trn):
    #     # ground truth plot
    #     fig_gt = transformed_dataset.show(batch)
    ...
    #     fig_recon = transformed_dataset.show(reconstructed_batch)
